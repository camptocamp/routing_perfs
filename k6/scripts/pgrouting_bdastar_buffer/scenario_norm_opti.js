  ...
  "queryString=\
    SELECT * FROM pgr_bdAstar(\
        '\
            ...
            FROM idf_2po_4pgr\
            WHERE\
              clazz in (11, 12, 13, 14, 15, 16, 21, 22, 31, 32)\
              AND\
              ST_Intersects(\
                geom_way,\
                (\
            ...
            WHERE\
              ST_Intersects(\
                geom_way,\
                (\
                SELECT ST_Buffer(v1.geom_vertex, 0.05)\
                FROM idf_2po_vertex v1\
                WHERE v1.id = __FROM_ID__\
                )\
              )\
            UNION\
            SELECT\
            ...
            WHERE\
              ST_Intersects(\
                geom_way,\
                (\
                SELECT ST_Buffer(v2.geom_vertex, 0.05)\
                FROM idf_2po_vertex v2\
                WHERE v1.id = __TO_VID__\
                )\
              )\
        ',\
        __FROM_ID__,\
