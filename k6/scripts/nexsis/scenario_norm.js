...
export function m1_campagne() {
  ...
  requests = vehicles_m1_campagne.map((vehicle) => {
    url = urlTemplate;
    url = url.replace(/__FROM_ID__/g, vehicle.id);
    url = url.replace(/__TO_VID__/g, intervention_m1_campagne.id);
    url = url.replace(/__RADIUS_DEG__/g, 0.1);
...
const urlTemplate =
  // "http://localhost:8084/temps-transit-dynamique/sequence-itineraire?startVid=__FROM_ID__&endVid=__TO_VID__&bufferRadius=__RADIUS_DEG__";
  "https://api.kube.dev.nexsis.ansc.minint.fr/itineraires/nexsis-2748-route-sequence-itineraire/temps-transit-dynamique/sequence-itineraire?startVid=__FROM_ID__&endVid=__TO_VID__&bufferRadius=__RADIUS_DEG__";
